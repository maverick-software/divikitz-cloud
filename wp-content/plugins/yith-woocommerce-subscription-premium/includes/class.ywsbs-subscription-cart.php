...
if ( ! class_exists( 'YWSBS_Subscription_Cart' ) ) {
	...
	class YWSBS_Subscription_Cart {
		...
		public function disable_gateways( $gateways ) {
			$subscription_on_cart = false;
			if ( WC()->cart && !WC()->cart->is_empty() && is_checkout() ) {
				$subscription_on_cart = self::cart_has_subscriptions();
			} elseif ( 0 < get_query_var( 'order-pay' ) ) {
				...
				$subscription_on_cart = 0 === count( $subscription_on_cart ) ? false : $subscription_on_cart;
			}

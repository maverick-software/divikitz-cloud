 ...
 * @package YITH WooCommerce Stripe Connect
 * @version 1.0.0
 */
...
if ( ! class_exists( 'YITH_Stripe_Connect_Renew_Needs_Action_Email' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class YITH_Stripe_Connect_Renew_Needs_Action_Email extends WC_Email {
		 ...
		 *
		 * @return \YITH_Stripe_Connect_Renew_Needs_Action_Email
		 * @since 1.10.0
		 */
		public function __construct() {
			$this->id             = 'renew_needs_action';
			$this->title          = 'YITH WooCommerce Stripe Connect - ' . __( 'Payment confirmation email', 'yith-stripe-connect-for-woocommerce' );
			$this->description    = __( 'This email is sent to customers that have an off session payment pending confirmation', 'yith-stripe-connect-for-woocommerce' );
			$this->customer_email = true;

			$this->heading = __( 'Confirm your {order_total} payment', 'yith-stripe-connect-for-woocommerce' );
			$this->subject = __( 'Confirm your {order_total_plain} payment', 'yith-stripe-connect-for-woocommerce' );
...
If you have any question, contact us at {contact_email}\n\n" );
			...
			add_action( 'yith_stripe_connect_renew_intent_requires_action_notification', array(
				$this,
				'trigger'
			), 10, 1 );
		...
		public function trigger( $order_id ) {
			...
			if ( $this->order && $user_id = $this->order->get_user_id() ) {
				...
				$tokens = WC_Payment_Tokens::get_tokens( array(
					'user_id'    => $user_id,
					'gateway_id' => YITH_Stripe_Connect::$gateway_id
				) );
		...
		public function get_content_html() {
			...
			/** APPLY_FILTERS: yith_stripe_connect_pay_renew_url_enabled
			*
			* Filter the use of the nonce in the renew_url.
			*/
			if ( apply_filters( 'yith_stripe_connect_pay_renew_url_enabled', defined( 'YITH_YWSBS_VERSION' ) && version_compare( YITH_YWSBS_VERSION, '1.6.1', '>=' ) ) ) {

				/** APPLY_FILTERS: yith_stripe_connect_pay_renew_url
				*
				* Filter the nonce of the pay_url.
				*
				* @param wp_nonce_url Default nonce.
				*/
				$pay_url = apply_filters( 'yith_stripe_connect_pay_renew_url', wp_nonce_url( $this->order->get_checkout_payment_url(), 'ywsbs_manual_renew', 'ywsbs_manual_renew' ) );
			}
			...
			$this->placeholders = array_merge(
				...
				array(
					...
					'{order_total_plain}' => strip_tags( wc_price( $order->get_total(), array( 'currency' => $order->get_currency() ) ) ),
					'{billing_email}'     => $this->order->get_billing_email(),

					/** APPLY_FILTERS: yith_stripe_connect_contact_email
					*
					* Filter the contact email.
					*
					* @param string option 'woocommerce_email_from_address' option.
					*/
					'{contact_email}'     => apply_filters( 'yith_stripe_connect_contact_email', get_option( 'woocommerce_email_from_address', '' ) ),
				)
			);
			...
			ob_start();
			yith_wcsc_get_template( $this->template_html, array(
				'order'         => $order,
				'username'      => $username,
				'pay_renew_url' => $pay_url,
			...
			*/
			...
			$this->placeholders = array_merge(
				...
				array(
					...
					'{card_last4}'        => $this->token->get_last4(),
					'{order_id}'          => $order->get_id(),
					'{order_total}'       => strip_tags( wc_price( $order->get_total(), array( 'currency' => $order->get_currency() ) ) ),
					'{order_total_plain}' => strip_tags( wc_price( $order->get_total(), array( 'currency' => $order->get_currency() ) ) ),
			...
			ob_start();
			yith_wcsc_get_template( $this->template_plain, array(
				'order'         => $order,
				'username'      => $username,
				'pay_renew_url' => $pay_url,
				'email_heading' => $this->get_heading(),
...
return new YITH_Stripe_Connect_Renew_Needs_Action_Email();